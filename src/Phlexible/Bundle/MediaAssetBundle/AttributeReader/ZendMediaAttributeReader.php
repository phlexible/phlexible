...
class ZendMediaAttributeReader implements AttributeReaderInterface
{
    ...
    public function read(FileInterface $file, MetaBag $metaBag)
    {
        ...
        try {
            ...
            $metaData
                ->set('channelmode', $mpegMode[$frame->getMode()])
                ->set('version', $mpegVersion[$frame->getVersion()])
                ->set('layer', $mpegLayer[$frame->getLayer()])
                ->set('emphasis', $mpegEmphasis[$frame->getEmphasis()])
                ->set('copyright', $mpegBoolean[$frame->getCopyright()])
                ->set('crc', $mpegBoolean[$frame->getCrc()])
                ->set('original', $mpegBoolean[$frame->getOriginal()]);
