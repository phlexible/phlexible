...
class RightsController extends Controller
{
        ...
        if (count($rightsData)) {
            ...
            if (count($userIds)) {
                ...
                $userSelect = $db->select()
                             ->from($db->prefix.'user', array(new Zend_Db_Expr('CONCAT("uid__", uid)'), new Zend_Db_Expr('CONCAT(firstname, " ", lastname) AS name')))
            ...
            if (count($groupIds)) {
                ...
                $groupsSelect = $db->select()
                             ->from($db->prefix.'group', array(new Zend_Db_Expr('CONCAT("gid__", gid)'), 'name'))
    ...
    protected function _getRightsForSubjects($contentType, $contentId, array $subjectsData,
                                             array $path, array $allRights, array $rightsData)
    {
        try
        {
            $subjects = array();
