...
class MetaSearch implements SearchProviderInterface
{
    ...
    public function search($query)
    {
        ...
        $select = $this->db
            ->select()
            ->from(array('f' => $this->db->prefix . 'mediamanager_files'), array('id', 'folder_id', 'name', 'create_user_id', 'create_time'))
            ->join(array('m' => $this->db->prefix . 'mediamanager_files_metasets_items'), 'm.file_id = f.id AND m.file_version = m.file_version', array())
