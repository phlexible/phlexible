...
Ext.provide('Ext.grid.filter.DateFilter');

Ext.require('Ext.grid.filter.Filter');
...
Ext.grid.filter.DateFilter = Ext.extend(Ext.grid.filter.Filter, {
    ...
    serialize: function () {
        ...
        if (this.dates.before.checked) {
            args = [
                {type: 'date', comparison: 'lt', value: this.getFieldValue('before').format(this.dateFormat)}
            ];
        }
