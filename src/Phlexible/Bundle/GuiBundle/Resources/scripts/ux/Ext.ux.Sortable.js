...
Ext.ux.Sortable = function (obj) {
    ...
    if (typeof this.dragGroups === 'string') {
        this.dragGroups = [this.dragGroups];
    }
    ...
    if (this.dragGroups.length > 1) {
        ...
        this.ddGroups[this.dragGroups[0]] = true;
    }
...
Ext.extend(Ext.dd.SortableDragZone, Ext.dd.DragZone, {
    onInitDrag: function (x, y) {
        this.sortableContainer.fireEvent('startsort', this, this.sortableContainer, this.dragData);

        var text = 'Move'; //Ext.get(this.dragData.ddel.id).child('legend span').dom.innerHTML;

        var dragged = document.createElement('div');
        dragged.appendChild(document.createTextNode(text));

        //'Move'; //this.dragData.ddel.cloneNode(true);
        dragged.id = '';
...
Ext.extend(Ext.dd.SortableDropZone, Ext.dd.DropZone, {
    ...
    notifyOver: function (dd, e, data) {
        if (this._testDDGroup()) {
            ...
            var a = Ext.getCmp(n.id);
            var b = Ext.getCmp(data.ddel.id);
    ...
    onNodeOver: function (n, dd, e, data) {
        if (this._testDDGroup()) {
            ...
            if (destEl.parent().id != this.srcEl.parent().id) {
                return this.dropNotAllowed;
            }
    ...
    notifyDrop: function (dd, e, data) {
        if (this._testDDGroup()) {
            if (this.srcEl !== null) {
                ...
                this.sortableContainer.fireEvent('endsort', this, this.sortableContainer, dd, e, data);
            }
            return true;
        }
    },
