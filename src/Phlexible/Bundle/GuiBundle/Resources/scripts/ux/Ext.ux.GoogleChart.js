...
Ext.provide('Ext.ux.GoogleChart');
     ...
     */, getChartData: function () {
        if (this.dataType == "text") {
            return this.getFormattedValue({
                value: this.data, name: '&chd', isMixed: true, initvalue: this.dataTypes[this.dataType]
            });
        } else if (this.dataType == "simple") {
            return this.getFormattedValue({
                value: this.data, isMixed: true, name: '&chd', mixedJoinChar: ',', initvalue: this.dataTypes[this.dataType], fn: function (item) {
                    return this.simpleEncode(item, this.maxValue || 100);
                }, fnscope: this, defaultvalue: '_'
            });
        } else {
            return this.getFormattedValue({
                value: this.data, isMixed: true, name: '&chd', mixedJoinChar: ',', initvalue: this.dataTypes[this.dataType], fn: function (item) {
                    return this.extendedEncode(item);
                }, fnscope: this, defaultvalue: '__'
            });
        }
    }
    /**
     ...
     */, getFormattedValue: function (config) {
        ...
        if (config) {
            if (config.value && config.name) {
                if (!config.isMixed) {
                    var fn = Ext.isArray(config.value) ? function (item) {
                        return item.join(config.joinChar ? config.joinChar : ',')
                    } : function (item) {
                        return item;
                    };
