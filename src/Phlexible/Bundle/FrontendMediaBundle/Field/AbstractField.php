...
class AbstractField extends BaseAbstractField
{
    ...
    protected function _getTemplateConfig($media, Media_Site_File_Abstract $file)
    {
        ...
        if (!empty($media['imageList'])) {
            foreach ($media['imageList'] as $imageTemplateRow) {
                ...
                $imageTemplate = MWF_Registry::getContainer()->get('mediatemplates.repository')->find(
                    $imageTemplateKey
                );
        ...
        if (!empty($media['videoList'])) {
            foreach ($media['videoList'] as $videoTemplateRow) {
                ...
                $videoTemplate = MWF_Registry::getContainer()->get('mediatemplates.repository')->find(
                    $videoTemplateKey
                );
        ...
        if (!empty($media['audioList'])) {
            foreach ($media['audioList'] as $audioTemplateRow) {
                ...
                $audioTemplate = MWF_Registry::getContainer()->get('mediatemplates.repository')->find(
                    $audioTemplateKey
                );
    ...
    protected function _getMediaData(Media_Site_File_Abstract $file)
    {
        ...
        try {
            ...
            $mediaTemp['readablesize'] = $formatter->formatFilesize($file->getSize());
            ...
            $mediaSiteManager = $container->get('phlexible_media_site.site_manager')->getByFileId($file->getID());
        ...
        foreach ($media['imageList'] as $imageTemplateRow) {
            ...
            if ($file !== null) {
                ...
                $imageTemp = array(
                    ...
                    'readablesize' => $formatter->formatFilesize($cacheItem->getFileSize()),
