    ...
    public function indexAction()
    {
        ...
        try
        {
            ...
            $select = $db->select()
                         ...
                         ->join(array('ev' => $db->prefix . 'element_version'), 'ev.eid = ed.eid AND ev.version = ed.version', array())
                         ->join(array('etv' => $db->prefix . 'elementtype_version'), 'etv.element_type_id = ev.element_type_id AND etv.version = ev.element_type_version', array())
                         ->join(array('ets' => $db->prefix . 'elementtype_structure'), 'ets.element_type_id = etv.element_type_id AND ets.version = etv.version AND ed.ds_id = ets.ds_id', array('media'))
                         ->where('ed.eid = ?', $eid)
            ...
            if (empty($media))
            {
                $select = $db->select()
                             ...
                             ->join(array('ev' => $db->prefix . 'element_version'), 'ev.eid = ed.eid AND ev.version = ed.version', array())
                             ->join(array('etv' => $db->prefix . 'elementtype_version'), 'etv.element_type_id = ev.element_type_id AND etv.version = ev.element_type_version', array())
                             ->join(array('ets' => $db->prefix . 'elementtype_structure'), 'ets.element_type_id = etv.element_type_id AND ets.version = etv.version', array())
                             ->join(array('etsr' => $db->prefix . 'elementtype_structure'), 'etsr.element_type_id = ets.reference_id AND etsr.version = ets.reference_version AND ed.ds_id = etsr.ds_id', array('media'))
                             ->where('ed.eid = ?', $eid)
                             ->where('ed.version = ?', $version)
                             ->where('ed.data_id = ?', $dataId);
            ...
            if (!empty($media))
            {
                ...
                foreach ($templateData as $templateKey => $templateRow)
                {
                    ...
                    $templateData[$templateKey]['readablesize'] = Brainbits_Format_Filesize::format($cacheItem->getFileSize());
